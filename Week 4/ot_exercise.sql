...
select region_id, count(country_id) from countries group by region_id order by count(country_id) desc limit 1;
...
---------------------------------------------------------------------------------------
select * from regions;
select region_name as 'region with most locations' from regions where region_id = (select region_id from countries group by region_id order by count(country_id) desc limit 1);
...
select state, city, postal_code from locations where country_id != "US" and city like 'S%';
...
select product_name, list_price from products where list_price between	100 and 500;
...
select product_name, list_price, category_name from products join product_categories on product_categories.category_id = products.category_id;
...
select product_name from products join product_categories on product_categories.category_id = products.category_id 
join inventories on inventories.product_id = products.product_id join warehouses on warehouses.warehouse_id = inventories.warehouse_id 
where category_name = 'cpu' and warehouse_name = 'Toronto' and quantity >= 100;
...
select category_name, round(avg(list_price),2) from products join product_categories on product_categories.category_id = products.category_id group by category_name;
...
select list_price, count(list_price) from products group by list_price order by count(list_price) desc limit 2;
...
select category_name, list_price, count(list_price) as priceCount from products join product_categories on products.category_id = product_categories.category_id
group by category_name, list_price having priceCount > 1 order by priceCount desc ;
...
select @rowindex:=@rowindex + 1 as rowindex, list_price from products order by list_price;
...
---------------------------------------------------------------------------------------
SET @rowindex := -1;
select avg(list_price) as 'median' from (select @rowindex:=@rowindex + 1 as rowindex, list_price from products order by list_price) as p
where p.rowindex IN ( FLOOR( @rowindex / 2 ), CEIL( @rowindex / 2 ) );
...
---------------------------------------------------------------------------------------
SET @rowindex := -1;
select avg(list_price) as 'median' from (select @rowindex:=@rowindex + 1 as rowindex, list_price from products where category_id = 2 order by list_price) as p
where p.rowindex IN ( FLOOR( @rowindex / 2 ), CEIL( @rowindex / 2 ) );
